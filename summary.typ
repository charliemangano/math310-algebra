...
*def ::* _group of units in $ZZ"/"n ZZ$_ :: $(ZZ"/"n ZZ, dot)$, $ ((ZZ"/"n ZZ)^*, dot) = {x in ZZ"/"n ZZ : exists x^(-1) in ZZ"/"n ZZ} "  (invertible)" $ #remark([$[a]_n in ZZ"/"n ZZ, [a]_n != [0]_n$, $ [a]_n "unit in" ZZ"/"n ZZ <=> gcd(a,n) = 1 $ $ |(ZZ"/"n ZZ)^*, dot| = phi(n) $]) #remark([$p in ZZ "prime" => (ZZ"/"n ZZ)^*, dot) "cyclic" and |(ZZ"/"n ZZ)^*, dot)| = p - 1$])
...
== quotient group.
*def ::* _quotient group_ :: $G$ and $N lt.tri G$, $ G"/"N = {(x N), forall x in G} "  (left N-cosets)" $ $ "with operation" (x N) dot_(G"/"N) (y N) = (x y N) $ $ e_(G"/"N) = 1N "and" (x N)^(-1) = x^(-1) N $ #remark([$phi.alt : G -> H$ homomorphism, $ G"/"ker(phi.alt) tilde.equiv im(phi.alt)$])
...
*def ::* _orbit_ :: $G$ acts on $E$ with operation $dot$, $forall x in E$, $ "orb"(x) = {g dot x, g in G} $ #remark($|"orb"(x)| = 1 => x "\"fixed point\"" $)
*def ::* _symmetric group_ :: $n in NN, n >= 1$ $ S_n =(rho, dot_S_n) " with" $ $ rho : {1, dots n} -> {1, dots n} "injective  (permutations)" $ $ e_S_n = rho : rho(i) = i and rho^(-1) : rho^(-1)(rho(i)) = i $ #remark([the symmetric group of order $n$ is the group of $rho$'s of order $n$, and $|S_n| = n!$ is the order of the group itself])
*def ::* _k-cycle_ :: $sigma in S_n$ permutation and $angle.l sigma angle.r subset S_n$ subgroup generated by $sigma$, $ exists ! i in {1 dots n} : |"orb"_sigma (i)| "non-trivial" in {sigma(i)}_(i in {1 dots n}) \ => sigma " "k"-cycle with" k := |"orb"_sigma (i)| $ #remark([_transposition_ :: 2-cycle]) #remark([_cycle notation_ :: $pi in S_n$ a $k$-cycle and $x in {1 dots n}$ in the non-trivial orbit of $pi$, $pi = (x" "pi(x)" "pi^2(x)" "dots" "pi^(k-1)(x))$ the cycle notation of $pi$])
*def ::* _disjoint cycles_ :: $pi_1, pi_2 in S_n$ $k$-cycles are disjoint if their non-trivial orbits don't intersect #remark([disjoint cycles commute in $S_n$])
...
*th ::* a permutation is a unique product of disjoint cycles, up to the order of factors #remark([every $k$-cycle in $S_n$ is a product of $k-1$ transposition not necessarily disjoint]) #remark([$(1" "2" "dots" "k) = (1" "k)(1" "k-1) dots (1" "3)(1" "2)$]) #remark([_cycle decomposition_ :: $pi,rho in S_n$, the cycle decomposition of $pi rho pi^(-1)$ is obtained by replacing every $i$ in the cycle decomposition of $rho$ by $pi(i)$])
