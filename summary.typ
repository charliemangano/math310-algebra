...
#item("cor", [$forall a,b in ZZ^*$, $ exists x,y in ZZ : gcd(a,b) = a x + b y $])
#item("cor", [$a,b in ZZ^*$ and $d = gcd(a,b)$ $ a x + b y = c, c in ZZ^* "has integer solution" <=> c in d ZZ $])
...
#item("cor", [$G$ finite, $ forall g in G, exists k in NN : |G| = k o(g) $])
#item("cor", [$G$ finite and $g in G$, $ g^(|G|) = e $])
#item("cor", [$G$ finite, $ |G| = p "prime" => G "cyclic" $])
...
#item("cor", [$S_n$ is generated by ${(i j)}_(1 <= i < j <= n)$])
...
#item("cor", [$G$ finite abelian, $ exists p in NN, p "prime" : G tilde.equiv C_p  $])
...
#item("cor", [$A = ZZ"/"n ZZ, A "field" <=> n "prime"$])
...
#item("cor", [$I = ({a_1, dots, a_n}) subset ZZ$ ideal, $ I = (d) subset ZZ "where" d = gcd(a_1, dots, a_n) $])
...
#remark([$c_A = m k in ZZ^plus: m,k >= 2 => exists "non-trivial zero divisors" in A$])
#item("cor", [$A "field" => c_A = e_plus or c_A = p, p "prime"$ ])
...
#item("cor", [$m,n in ZZ$ and $gcd(m,n) = 1$, $ forall a_1,a_2 in ZZ, exists a in ZZ : a equiv a_1 (mod m) and a equiv a_2 (mod n) \ a "solution" => {a + m n ZZ} "solutions"  $])
...
#item("cor", [$d_1, dots, d_n in ZZ : forall i != j, gcd(d_i,d_j) = 1$ and $d = d_1 dots d_n$, $ forall a_1, dots, a_r in ZZ, exists a in ZZ : cases(a equiv a_1 (mod d_1), dots, a equiv a_r (mod d_r)) \ a "solution" => {a plus d ZZ} "solutions" $])

== polynomials in one variable with coefficients in commutative ring
#item("def", name: "ring of polynomials", [$A$ commutative ring, $ A[x] &= {a_0 + a_1 x + dots + a_n x^n, n in NN, a_i in A} \ &= {(a_0, a_1, dots)}_(a_i in A) : a_i = 0 "for large" i in NN $])
#remark([$A[x]$ commutative ring])
#item("def", name: "degree", [$f(x) in A[x]$ non-trivial, $ deg(f(x)) = n "largest" : a_n != 0 $])
#remark([$a_n$ dominant coefficient and $a_0$ constant term])
#remark([$f(x) = 0 => deg(f) := -infinity$])
#item("prop", [$A[x]$ ring and $f,g in A[x]$ $ deg(f + g) <= max{deg(f), deg(g)} \ A "integral domain" => deg(f dot g) = deg(f + g) $])
#item("th", [$A$ integral domain, $ A[x] "integral domain" \ "invertible elements" in A[x] = "invertible elements" in A $])
#item("th", [$F$ field, $f,d in F[x]$ and $deg(d) >= 1$, $ exists q,r in F[x] : f(x) = q(x)d(x) +r(x) "with" \ r(x) = 0 or deg(r) < deg(d) $])

== euclidean domains. principal ideal domains.
#item("def", name: "euclidean domain", [$A$ integral domain, $ exists nu : A without {e_plus} -> NN : forall a,b in A, b != e_plus, exists q,r in A : a = q b + r \ r = e_plus or nu(r) < nu(b) $])
#remark([$F "field" => F[x] "euclidean domain"$])
#item("th", [$ A "euclidean domain" => A "principal ideal domain" $])
#item("cor", [$ F "field" => F[x] "principal ideal domain" $])
#item("def", name: "associates", [$A$ integral domain and $a,b in A$, $ b = a u, u in A^* \ a = b v, v in A^* $])
#remark([ideals generated by associates are the same])
#item("prop", [$A$ integral domain, $forall a,b,d_1,d_2,l_1,l_2 in A$, $ d_1,d_2 = gcd(a,b) => d_1, d_2 "associates" \ l_1,l_2 = "lcm"(a,b) => l_1,l_2 "associates" $])
#remark([_bezout's theorem_ *::* $E$ euclidean domain, $d = gcd(a,b) => (a) + (b) = (d) "ideal" subset E$])
#remark([$a,b,c in E$ euclidean domain, $gcd(a,b) = 1 and a|b c => a|c$ and $gcd(a,b) = 1  and gcd(a,c) = 1 => gcd(a, b c) => 1$ ])
#remark([$a,b,c in E$ euclidean domain, $gcd(a,b) = 1 and a|c and b|c => a b|c$ and $gcd(a,b) = 1 => "lcm"(a,b) = a b$])
#remark([$(a) sect (b) = (m) "ideal" subset E, "with" m = "lcm"(a,b)$])

== chinese remainder theorem for euclidean domain.
#item("th", [$A$ euclidean domain and $m = m_1 dots m_r "with" gcd(m_i,m_j) = 1$, $ f : A\/(m) -> A\/(m_1) times dots times A\/(m_r) "isomorphism, with" \ f(macron(x)_((m))) = (macron(x)_((m_1)), dots, macron(x)_((m_r))) $])
#item("cor", name: "chinese remainder theorem for polynomial rings", [$F$ field and $f_1(x), dots, f_r(x)$ polynomials in $F[x]$, $ gcd(f_i,f_j) = 1 \ => exists Phi : F[x]\/(f_1 dot dots dot f_r) -> F[x]\/(f_1) times dots times F[x]\/(f_r) "isomorphism" $]) 

== irreducible elements in euclidean domains.
#item("def", name: "irreducible", [$A$ integral domain and $a,b,c in A$, $ c in.not A^* and c != e_plus and [c = a b => a in A^* or b in A^*] $])
#item("th", [$A$ principal ideal domain, $ p in A "irreducible" <=> p != e_plus and (p) subset A "maximal" $])
#item("prop", [$a,b in A$ euclidean domain and $I = (a) subset.neq A $ non-trivial, $ macron(b) in (A\/I)^* <=> gcd(a,b) = 1 \ macron(b) in A\/I "non-trivial zero divisor" <=> b in.not I and gcd(a,b) != 1 \ A\/I "field" <=> a in A "irreducible" $])
#item("cor", [$F$ field and $f in F[x]$ non-trivial polynomial, $ F[x]\/(f) "field" <=> f "irreducible in" F[x] $])
#remark([$ &&"division ring" &subset wide med "domain" &&subset "ring" \ &&"field" subset "euclidean domain" subset "principal ideal domain" &subset "integral domain" &&subset "commutative ring" $])
#remark([$ZZ[x] and F[x,y]$ with $F$ field are integral domains])
#remark([$n$ not prime, $ZZ\/n ZZ$ and $(ZZ\/n ZZ)[x]$ not integral domains])

== quotient of polynomial rings.
#item("prop", [$F$ field and $f in F[x]$ polynomial, $ deg(f) = 1 => f "irreducible" \ deg(f) = 2 or deg(f) = 3, f "irreducible" <=> "no root"in F $ ])
#item("prop", [$alpha = r/s in QQ$ root of $f in ZZ[x]$, $ s|a_n and r|a_0 $ ])
#item("prop", name: "eisenstein's criterion", [$f in ZZ[x]$ polynomial with $gcd(a_0,dots,a_n) = 1$, $ p in ZZ : p divides {a_0, dots, a_(n-1)} and p divides.not a_n and p^2 divides.not a_0 => f "irreducible in" QQ[x] "and" ZZ[x] $])
#item("prop", [$F$ field and $f in F[x]$ irreducible polynomial with $deg(f) = n >= 1$, $ K = F[x]\/(f) "field" : forall a in K, a = a_0macron(1) + dots + a_(n-1)overline(x^(n-1)) \ "where" a_i in F and overline(x^i) "congruence class" x^i + (f) $])
#item("cor", [$F$ field, $|F| = q$ and $f in F[x]$ irreducible polynomial, $ deg(f) := n >= 1 => |F[x]\/(f)| = q^n $])

== finite fields.
#item("def", name: "notation", [$p in NN$ prime, $ FF_p equiv ZZ\/p ZZ $])
#item("prop", [$K$ field and $n in NN^+$, $ |K| = p^n => c_K = p $])
#item("prop", [$A$ field, $ |A| = p => A tilde.equiv FF_p $])
#item("prop", [$K$ field, $ c_K = p => exists L "subfield" subset K : L tilde.equiv FF_p $])
#item("prop", [$K$ finite field, $ c_K = p => exists n in NN^+ : |K| = p^n $])
#item("prop", [$F$ field and $f in F[x]$ polynomial, $ exists K "field" : F subset K and "roots of" f in K $])
#item("prop", [$K$ finite field, $ K^* "cyclic" $])
#item("th", [$p$ prime and $n in NN, n > 1$, $ exists! K "field": |K| = p^n \ exists f in FF_p [x] "irreducible" : FF_p [x]\/(f) tilde.equiv K $])
#item("cor", [$forall n in NN^+, forall p "prime"$, $ exists f in FF_p [x] "irreducible" : deg(f) = n $])
#item("prop", [$forall n in NN^+, forall p "prime"$, $ exists! FF_(p^n) "finite field" : |FF_(p^n)| = p^n and c_(FF_(p^n)) = p $])
